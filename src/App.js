...
class App extends Component {
  ...
  componentDidMount() {
    this.state.globalCryptoStats
      .ready()
      .then(() => this.setGlobalCryptoStats());
  ...
  setCryptoStatList = (currency) => {
    ...
    topCryptoStats
      ...
      .then(() => {
        this.setState({ topCryptoLoaded: true, currency: currency });
      });
  };
  ...
  rankSorter = (cryptoA, cryptoB) => cryptoA.cmc_rank - cryptoB.cmc_rank;
  marketCapSorter = (cryptoA, cryptoB) =>
    cryptoA.quote.USD.market_cap - cryptoB.quote.USD.market_cap;
  priceSorter = (cryptoA, cryptoB) =>
    cryptoA.quote.USD.price - cryptoB.quote.USD.price;
  volumeSorter = (cryptoA, cryptoB) =>
    cryptoA.quote.USD.volume_24h - cryptoB.quote.USD.volume_24h;
  changeSorter = (cryptoA, cryptoB) =>
    cryptoA.quote.USD.percent_change_24h - cryptoB.quote.USD.percent_change_24h;
  nameSorter = (cryptoA, cryptoB) =>
    cryptoA.name.toLowerCase() > cryptoB.name.toLowerCase() ? 1 : -1;
  ...
  filterRank = (cryptoList) => {
    return cryptoList.filter((crypto) => {
      return crypto.cmc_rank <= this.state.showOnly;
    });
  };
  ...
  render() {
    ...
    const renderBTCPriceChart = !btcChartLoaded ? (
      loading
    ) : (
      <BTCPriceChart chartXCoords={chartXCoords} chartYCoords={chartYCoords} />
    );

    const sectionSwap = displayTopCrypto
      ? renderCryptoTable
      : renderBTCPriceChart;
